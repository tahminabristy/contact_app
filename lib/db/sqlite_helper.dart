...
class SqliteHelper {
  ...
  $tbl_contact_col_favorite integer)''';
  ...
  static Future<int> updateContactFavorite(int? id, int favValue) async {
    final db = await open();
    return db.update(tbl_contact, {tbl_contact_col_favorite : favValue}, where: '$tbl_contact_col_id = ?', whereArgs: [id]);
  }
  ...
  static Future<List<ContactModel>> getAllFavoriteContacts() async {
    final db = await open();
    List<Map<String,dynamic>> mapList = await db.query(tbl_contact, where: '${tbl_contact_col_favorite} = ?', whereArgs: [1]);
    return List.generate(mapList.length, (index) => ContactModel.fromMap(mapList[index]));
  }
